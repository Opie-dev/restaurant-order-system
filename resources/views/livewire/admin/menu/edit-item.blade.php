                                        ...
                                        <div class="border rounded-lg p-3 {{ ($group['enabled'] ?? false) ? '' : 'opacity-60' }}">
                                            ...
                                            <div class="space-y-2">
                                                @foreach(($group['options'] ?? []) as $oIndex => $opt)
                                                    <label wire:key="preview-opt-{{ $gIndex }}-{{ $oIndex }}" class="flex items-center gap-3 p-2 border rounded {{ !($opt['enabled'] ?? false) ? 'opacity-60 bg-gray-50 text-gray-400 cursor-not-allowed' : 'hover:bg-gray-50' }}">
                                                        @php $multiple = ($group['rules'][1] ?? 'one') === 'multiple'; @endphp
                                                        @if($multiple)
                                                            <input type="checkbox" class="w-4 h-4 text-purple-600" @disabled(!($opt['enabled'] ?? false)) />
                                                        @else
                                                            <input type="radio" name="preview-opt-{{ $gIndex }}" class="w-4 h-4 text-purple-600" @disabled(!($opt['enabled'] ?? false)) />
                                                        @endif
                                        ...
                                        <div class="border rounded-lg p-3 {{ ($group['enabled'] ?? false) ? '' : 'opacity-60' }}">
                                            ...
                                            <div class="space-y-2">
                                                @foreach(($group['options'] ?? []) as $oIndex => $opt)
                                                    ...
                                                    <label wire:key="preview-addon-{{ $gIndex }}-{{ $oIndex }}" class="flex items-center gap-3 p-2 border rounded {{ !($opt['enabled'] ?? false) ? 'opacity-60 bg-gray-50 text-gray-400 cursor-not-allowed' : 'hover:bg-gray-50' }}">
                                                        @php $multiple = ($group['rules'][1] ?? 'one') === 'multiple'; @endphp
                                                        @if($multiple)
                                                            <input type="checkbox" class="w-4 h-4 text-purple-600" @disabled(!($opt['enabled'] ?? false)) />
                                                        @else
                                                            <input type="radio" name="preview-addon-{{ $gIndex }}" class="w-4 h-4 text-purple-600" @disabled(!($opt['enabled'] ?? false)) />
                                                        @endif
                                                        <span class="flex-1 font-medium text-sm">{{ $opt['name'] ?? '' }}</span>
                                                        @if(isset($opt['price']))
                        ...
                        <div class="space-y-2">
                            @foreach($group['options'] ?? [] as $optionIndex => $option)
                                <div class="flex items-center gap-3" wire:key="opt-{{ $groupIndex }}-{{ $optionIndex }}">
                                    <input type="text" wire:model.live="options.{{ $groupIndex }}.options.{{ $optionIndex }}.name" placeholder="Option name" class="flex-1 border rounded px-3 py-2" />
                                    <div class="flex items-center gap-2">
                                        <input type="checkbox" wire:model.live="options.{{ $groupIndex }}.options.{{ $optionIndex }}.enabled" class="rounded" />
                                        <span class="text-xs text-gray-600">Enabled</span>
                                    </div>
                                    <button type="button" wire:click="removeOptionOption({{ $groupIndex }}, {{ $optionIndex }})" class="text-red-600 hover:text-red-800 p-1">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                        </svg>
                                    </button>
                            ...
                            <div class="flex items-center justify-between mt-1">
                                <button type="button" wire:click="addOptionOption({{ $groupIndex }})" class="text-blue-600">+ Add Option</button>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" wire:model.live="options.{{ $groupIndex }}.enabled" class="w-4 h-4 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-500 focus:ring-2" />
                                    <span class="text-sm font-medium text-gray-700">Enable this option group</span>
                                </label>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-2">
                            @foreach($group['options'] ?? [] as $optionIndex => $option)
                                <div class="flex items-center gap-2" wire:key="addon-{{ $groupIndex }}-{{ $optionIndex }}">
                                    <input type="text" wire:model.live="addons.{{ $groupIndex }}.options.{{ $optionIndex }}.name" placeholder="Option name" class="flex-1 border rounded px-3 py-2" />
                                    <input type="number" step="0.01" wire:model.live="addons.{{ $groupIndex }}.options.{{ $optionIndex }}.price" placeholder="Price" class="w-20 border rounded px-3 py-2" />
                                    <div class="flex items-center gap-2">
                                        <input type="checkbox" wire:model.live="addons.{{ $groupIndex }}.options.{{ $optionIndex }}.enabled" class="rounded" />
                                        <span class="text-xs text-gray-600">Enabled</span>
                                    </div>
                                    <button type="button" wire:click="removeAddonOption({{ $groupIndex }}, {{ $optionIndex }})" class="text-red-600 hover:text-red-800 p-1">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                        </svg>
                                    </button>
                            ...
                            @enderror
                            <div class="flex items-center justify-between mt-1">
                                <button type="button" wire:click="addAddonOption({{ $groupIndex }})" class="text-blue-600">+ Add Option</button>
                                <label class="flex items-center gap-2 cursor-pointer">
                                    <input type="checkbox" wire:model.live="addons.{{ $groupIndex }}.enabled" class="w-4 h-4 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-purple-500 focus:ring-2" />
                                    <span class="text-sm font-medium text-gray-700">Enable this addon group</span>
                                </label>
                            </div>
                        </div>
                    </div>
