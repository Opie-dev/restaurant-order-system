 <!-- Store Cover Image (mobile/desktop logic) -->
    ...
    @if($store)
        <div class="relative bg-white shadow {{ $mobileCover ? 'h-64 lg:h-80 overflow-hidden' : 'mb-6' }}">
            {{-- Background --}}
            @if($mobileCover)
            ...
            @else
                <div class="w-full bg-white rounded-b-2xl shadow-lg overflow-hidden flex items-end"></div>
            @endif
            ...
            <div class="absolute top-4 right-4">
                <nav class="flex items-center space-x-4">
                    ...
                    @else
                        ...
                        class="flex items-center gap-2 @if($mobileCover) text-white @else text-black @endif hover:bg-dark hover:bg-opacity-20 px-3 py-2 rounded-lg transition-colors font-medium backdrop-blur-sm">
                            Login
                        </a>
                    @endauth
                </nav>
            ...
            <div class="{{ $mobileCover ? 'absolute bottom-0 left-0 right-0 p-6 text-white' : 'relative flex items-center space-x-6 p-6 w-full' }}"
                ...
                @if($store->logo_path)
                    <img src="{{ Storage::url($store->logo_path) }}" alt="{{ $store->name }}"
                        class="{{ $mobileCover ? 'hidden sm:block h-16 w-16 bg-white bg-opacity-90 p-2 shadow-md' : 'h-20 w-20' }} object-contain rounded-lg ">
                @else
                ...
                <div>
                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 mb-1">
                        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold {{ $mobileCover ? 'text-white' : 'text-black' }}">
                            {{ $store->name }}
                        </h1>
                    ...
                    <p class="{{ $mobileCover ? 'text-gray-100 text-sm' : 'text-sm' }}">{{ $store->address }}</p>
